...
//        Constants.MIN_CHILDREN = Constants.MAX_CHILDREN/3;      // m = M/3
    ...
    /**
     * traverse the R tree and compute digest for each node
     * @return: String, digest of the root nood
     * @param: RStarNode, the root of R tree
     * @throws: null
     */
    @SuppressWarnings("finally")
	public String computeAllDigest(RStarNode root) {
        if(root.isLeaf()) {
            StringBuilder sb = new StringBuilder();
            for(Long pointer : root.childPointers) {
                  PointDTO dto  = storage.loadPoint(pointer);
                  sb.append(dto.digest);
            }
            root.setDigest(DigestUtil.digest(sb.toString(), "sha-1"));
            return root.getDigest();
        }else{
        	try {
        		StringBuilder sb = new StringBuilder();
        		for(Long pointer : root.childPointers) {
        			RStarNode node = storage.loadNode(pointer);
        			String digest = computeAllDigest(node);
        			sb.append(digest);
        		}
        		root.setDigest(DigestUtil.digest(sb.toString(), "sha-1"));
        	}catch(FileNotFoundException e){
